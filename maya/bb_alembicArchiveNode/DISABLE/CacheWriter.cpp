...
)
{
...

    {
        ...
        const float* srcPositions = fPositions->data();
    ...
    fNumVerts(0)
{
	...
	MStatus status;
    MHWRender::MGeometryExtractor extractor(geomRequirements, dagPath, true, &status);
...
CacheMeshSampler::CacheMeshSampler(const bool needUVs)
    : fNeedUVs(needUVs), fIsAnimated(true)
{}
...
bool CacheMeshSampler::addSampleFromMesh(MFnMesh& mesh)
{
    ...
    return fAttributeSet.updateAnimatedChannels(
        fIsAnimated, AttributeSet(mesh, fNeedUVs), path);
}
