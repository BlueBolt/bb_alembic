...
public:
    ...
    void readValue(index_t idx)
    {
        ...
        this->fValue = Value();
        {
            ...
            this->fValue = ArrayRegistry<BaseType>::lookup(key.digest, size);
...
public:
    ...
    typedef boost::shared_ptr<Array<BaseType> >
    ...
    void readValue(index_t idx)
    {
        ...
        if (it != fsConvertionMap.end()) {
            tbb::mutex::scoped_lock lock(ArrayRegistry<BaseType>::mutex());
            this->fValue = ArrayRegistry<BaseType>::lookup(it->second, size);
...
protected:
    ...
    static boost::shared_ptr<Array<IndexBuffer::index_t> >
...
private:
    ...
    boost::shared_ptr<Array<float> > convertMultiIndexedStream(
        boost::shared_ptr<Array<float> > attribArray,
        boost::shared_ptr<Array<IndexBuffer::index_t> > indexArray);
    ...
    boost::shared_ptr<Array<float> >                fCheckedNormals;
    boost::shared_ptr<Array<IndexBuffer::index_t> > fCheckedNormalIndices;
    Alembic::AbcGeom::GeometryScope                 fCheckedUVsScope;
    boost::shared_ptr<Array<float> >                fCheckedUVs;
    boost::shared_ptr<Array<IndexBuffer::index_t> > fCheckedUVIndices;

    // compute in computeNormals()
    Alembic::AbcGeom::GeometryScope                 fComputedNormalsScope;
    boost::shared_ptr<Array<float> >                fComputedNormals;
    boost::shared_ptr<Array<IndexBuffer::index_t> > fComputedNormalIndices;

    // compute in convertMultiIndexedStreams()
    size_t                                          fNumVertices;
    boost::shared_array<unsigned int>               fVertAttribsIndices;
    boost::shared_ptr<Array<IndexBuffer::index_t> > fMappedFaceIndices;

    // compute in remapVertAttribs()
    boost::shared_ptr<Array<float> >                fMappedPositions;
    boost::shared_ptr<Array<float> >                fMappedNormals;
    boost::shared_ptr<Array<float> >                fMappedUVs;
        
    // compute in computeWireIndices()
    boost::shared_ptr<Array<IndexBuffer::index_t> > fWireIndices;

    // compute in triangulate()
    boost::shared_ptr<Array<IndexBuffer::index_t> > fTriangleIndices;
};
...
private:
    ...
    boost::shared_ptr<Array<IndexBuffer::index_t> > fTriangleIndices;
    boost::shared_ptr<Array<IndexBuffer::index_t> > fWireIndices;
    boost::shared_ptr<Array<float> >                fPositions;
    boost::shared_ptr<Array<float> >                fNormals;
    boost::shared_ptr<Array<float> >                fUVs;
};
...
#else
    friend boost::shared_ptr<AlembicCacheReader> boost::make_shared<AlembicCacheReader>(
