    ...
    const boost::shared_ptr<const ShapeSample>&  prev)
{
    ...
    if (sample->normals() != prev->normals()) {
        fillNormalSample(meshSample, sample, prev->normals());
    }
    
    if (sample->uvs() != prev->uvs()) {
        fillUVSample(meshSample, sample, prev->uvs());
    }
...
void AlembicMeshWriter::fillWireframeSample(
    ...
    const boost::shared_ptr<const ShapeSample>& sample)
{
    // wrap the wireframe index sample, no memcpy occur
    // wireframe index is stored as custom property
    const int* wireIndexArray = (const int*)(sample->wireVertIndices() ?
                                    sample->wireVertIndices()->data() : NULL);
	...
	const boost::shared_ptr<const ShapeSample>& sample)
{
	...
	for(size_t i=0; i<sample->numIndexGroups(); ++i) {
		const int* faceIndicesCCW = (const int*)(sample->triangleVertIndices(i) ? 
                                        sample->triangleVertIndices(i)->data() : NULL);
...
void AlembicMeshWriter::fillPositionSample(
    ...
    const boost::shared_ptr<const ShapeSample>& sample)
{
    // wrap the sample, no memcpy occur
    const Alembic::Abc::V3f* positionArray = (const Alembic::Abc::V3f*)(sample->positions() ?
                                                sample->positions()->data() : NULL);
    ...
    bool forceWrite)
{
    ...
    if (sample->normals()) {
        ...
        const Alembic::Abc::N3f* normalArray =
            (const Alembic::Abc::N3f*)(sample->normals()->data());
    ...
    bool forceWrite)
{
    if (sample->uvs()) {
        ...
        const Alembic::Abc::V2f* uvArray =
            (const Alembic::Abc::V2f*)(sample->uvs()->data());
