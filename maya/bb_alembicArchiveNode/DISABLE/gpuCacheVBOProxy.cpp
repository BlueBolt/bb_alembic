    ...
    const bool isTemporary)
{
    ...
    if (!result) {
        result = boost::make_shared<VBOBuffer>(
            kIndexBufferType, buffer->array()->key(), buffer->array()->get());
        if (!isTemporary)
    ...
    const bool isTemporary)
{
    ...
    if (!result) {
        result = boost::make_shared<VBOBuffer>(
            kVertexBufferType, buffer->array()->key(), buffer->array()->get());
        if (!isTemporary)
            theBufferRegistry().insert(result);
    }
    ...
    switch (fLastBindingType) {
        case kPrimitives: 
            switch (bindingType) {
                ...
                case kVertexArrays:
                    gGLFT->glEnableClientState(MGL_VERTEX_ARRAY);
                    gGLFT->glVertexPointer(3, MGL_FLOAT, 0, positions->data());
                    
                    if (normals) {
                        gGLFT->glEnableClientState(MGL_NORMAL_ARRAY);
                        gGLFT->glNormalPointer(MGL_FLOAT, 0, normals->data());
                    }

                    if (uvs) {
                        gGLFT->glEnableClientState(MGL_TEXTURE_COORD_ARRAY);
                        gGLFT->glTexCoordPointer(2, MGL_FLOAT, 0, uvs->data());
                    }
                    break;
        ...
        case kPrimitives:
            {
                ...
                const unsigned int* const indices = fIndices->data();
                const float* const verts = fPositions->data();
        ...
        case kPrimitives:
            {
                ...
                const unsigned int* const indices = fIndices->data();
                const float* const verts = fPositions->data();
                const float* const norms = fNormals ? fNormals->data() : 0;
                const float* const uvs   = fUVs     ? fUVs->data()     : 0;
    ...
    case kVertexArrays:
        {
            ...
            gGLFT->glDrawElements(MGL_LINES, 24, MGL_UNSIGNED_INT, fIndices->data());
