...
boost::shared_ptr<Array<typename AlembicArray<ArrayProperty>::T> >
...
RawDataProvider::getSample(double seconds)
{
    ...
    const boost::shared_ptr<Array<IndexBuffer::index_t> > indexBuffer =
        fFaceIndicesCache.getValue();
    if (fGroupSizesCache.valid()) {
        const boost::shared_ptr<Array<IndexBuffer::index_t> > groupSizes =
            fGroupSizesCache.getValue();
        for(size_t i=0, offset=0; i<groupSizes->size(); offset+=3*groupSizes->get()[i], ++i) {
            triangleVertIndices.push_back(
                IndexBuffer::create(
                    indexBuffer, offset, offset+3*groupSizes->get()[i]));
        }
    }
    else {
...
boost::shared_ptr<Array<IndexBuffer::index_t> >
...
boost::shared_ptr<Array<float> > Triangulator::convertMultiIndexedStream(
    ...
    : fFile(file)
{
    ...
    try {
        ...
        fAbcArchive = factory.getArchive(fileName.asChar());
    }
